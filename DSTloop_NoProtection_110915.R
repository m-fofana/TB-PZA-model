...
#SA: Setting treatment outcomes of RIF/PZAr strains equal to RIFr on 2nd-line
...
init<-read.csv(paste("Baseline/Init/dstInit110915_Baseline", taskid, ".csv", sep=""))
...
#To change timesteps
totaltime<-120 #Total # of years simulated (120 if starting from beginning, set to 60 if starting in 2015)
...
for(loop in 1:sampleno){
  ...
  #dstlhs1[loop,19]<-dstlhs1[loop,17] #RIF/PZAr, 2nd-line; set same as RIFr
  #dstlhs1[loop,31]<-0 #Remove mox resistance acquisition on 2nd-line regimen for RIFr
  #dstlhs1[loop,34] <-0 #Remove Mox resistance acquistion on 2nd-line regimen for RIF/PZAr
...
dstlhs1[loop,34]<-dstlhs1[loop,31]# RIF/PZA->3x, STR
  ...
  # Times at which the model will be evaluated:
    times <- seq(0, 120, by = increment) #Complete duration of simulation; set first term to 0 if starting from beginning, 61 if starting in 2015
  
  out <- ode(statevect, times, TBdx, mastervect) #ODE solver function   
  
  Countermat<-array(dim=c(121,25)) #Create empty counter matrix
  for(i in 1:121){ #set to 1:121 if starting from beginning, 62:121 if starting in 2015
    Countermat[i,]<-Counter(out[(i),], #Set to i if starting from beginning, i-61 if from 2015
...
write.csv(dstlhs2,file=paste("DST2/NoProtection/Init/dstInit110915_NoProtect", taskid, ".csv", sep=""))
write.csv(dstlhs3,file=paste("DST2/NoProtection/Final/dstFinal110915_NoProtect", taskid, ".csv", sep=""))
write.csv(DSTlhsData,file=paste("DST2/NoProtection/Data/dstData110915_NoProtect", taskid, ".csv", sep=""))
write.csv(keep, file=paste("DST2/NoProtection/Keep/dstKeep110915_NoProtect", taskid, ".csv", sep=""))

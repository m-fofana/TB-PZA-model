#Mariam Fofana
#Initial steady state for DST model
#Last updated: October 2015

setwd("~/DST2")
source("Rcode/DSTfxns_102715.R")
library(rootSolve)
library(deSolve)
init<-matrix(nrow=3,ncol=48)
cond<-matrix(c(4000,1000,3000,2000,2000,3000),2,3)
...
for(ss in 1:3){
  ...
  intro<-100000 #Time of introduction of FQ-based regimen (If this is >totaltime, then FQ-based regimen is never implemented)
  ...
  for(loop in 1:sampleno){ 
    ...
    #Beta
    dstlhs1[loop,50]<-qunif((ss*0.25),8,12)#9.5,15.5) #Transmission parameter, centered around 12
  }
...
dstlhs2<-matrix(ncol=(50+12+10), nrow=sampleno) #Matrix to hold 50 sampled values + 11 output var + 9 selection var
...
#   S<-39158.85  #Total susceptible
#   
#   #DS-TB
#   L1s<-53293.99 #Latent rx-naive
#   A1s<-155.40 #Active, rx-naive
#   Fs<-1.25 #Failing rx
#   L2s<-7376.60 #Latent, rx-experienced
#   A2s<-13.91 #Active, rx-experienced
#   
#   #Rif monoresistance 
#   L1r<-0
#   A1r<-0
...
#   names(statevect)<-statename #assign parameter names
  ...
  mastermat1<-(master(0,0,acq1,acq2,acq3,acq4,acq5,acq6,acq7,acq8,comp))
  
  mastermat2<-(master(0,0,acq1,acq2,acq3,acq4,acq5,acq6,acq7,acq8,comp))
  
  #After introduction of FQ-based regimen
  mastermat2b<-(master(0,0,acq1,acq2,acq3,acq4,acq5,acq6,acq7,acq8,comp))
  ...
  #Run to steady state
  res<-runsteady(c(95000,cond[,ss],rep(0,45)),c(1,100000),TBdx,mastervect,
                 stol=1e-10)
 ...
 write.csv(init, file="DST_SteadyState.csv")

##############################################################################

#Check yield with each set of initial conditions
sampleno<-250 #Set # of simulations to run; change as desired
dstlhs1 <-matrix(ncol=50,nrow=sampleno) #Create empty matrix to store sampled values
...
for(loop in 1:sampleno){ 
  ...
  #Beta
  dstlhs1[loop,50]<-runif(1,8,14) #Transmission parameter
}
...
for(set in 1:3){
  ...
  #GENERATE MATRIX TO COLLECT RELEVANT INPUT & OUTPUT DATA
  dstlhs2<-matrix(ncol=(50+12+12), nrow=sampleno) #Matrix to hold 50 sampled values + 11 output var + 9 selection var
  DSTlhsData<-array(0,dim=c(sampleno,(totaltime+1),52)) #3D array to hold # in each of 48 compartments + time (total 49 values)
  #Plus 3 incidence values (overall, MDR, Pre-XDR)
  #for each year simulated (totaltime + 1 for year 0) 
  #and each individual simulation (sampleno)
  ...
  for(loop in 1:sampleno){
    ...
    #"resistance multiplier"
    #Sets resistance aquisition probability for DR strains as multiple of vaues for DS-TB
    resmult<-dstlhs1[loop,21]
    ...
    #Population state matrix
    ########################
    statemat[1,1]<-init[set,1]
    
    statemat[2,1]<-init[set,2]
    statemat[2,2:8]<-0
    
    statemat[3,1]<-init[set,3]
    statemat[3,2:8]<-0
    
    
    statemat[4,1]<-init[set,4]
    statemat[4,2:8]<-0
    
    
    statemat[5,1]<-init[set,5]
    statemat[5,2:8]<-0
    
    
    statemat[6,1]<-init[set,6]
    statemat[6,2:8]<-0
  ...
  keep<-which(rowSums(dstlhs2[,63:74])==12) #Create vector to keep trak of which simulations meet criteria
  ...
  write.table(dstlhs2,file=paste("Steadyinit102715",set,".csv",sep=""), sep=",",append=F)
  write.table(dstlhs3,file=paste("Steadyfinal102715",set,".csv",sep=""),sep=",",append=F)
  write.csv(DSTlhsData,file=paste("SteadyData102715",set,".csv",sep=""))
  write.csv(keep, file=paste("SteadyKeep102715",set,".csv",sep=""))
